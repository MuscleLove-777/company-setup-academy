...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('company-setup-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('company-setup-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('company-setup-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('company-setup-progress', JSON.stringify(this.progress));
        localStorage.setItem('company-setup-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero" style="background: linear-gradient(135deg, #14532d 0%, #15803d 50%, #16a34a 100%);">
                    <h2>🏛️ 会社設立アカデミー へようこそ</h2>
                    <p>会社設立の構想から登記完了、届出まで――すべてを網羅した実践マニュアルで学びましょう。</p>
                </div>
    ...
    showCompletionMessage() {
        ...
        modal.innerHTML = `
            ...
            <p>
                全モジュールを完了しました！<br>
                会社設立に必要な知識を一通り学びました。<br>
                実際の設立にあたっては、司法書士・税理士等の専門家にもご相談ください。
            </p>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('company-setup-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('company-setup-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('company-setup-progress');
            localStorage.removeItem('company-setup-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
